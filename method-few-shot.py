...
def retrieve_dynamic_few_shots(target_sample: dict, pool: list[dict], k: int = 5) -> list[dict]:
    """
    SimICL 检索算法：从 pool 中检索与 target_sample 最相似的 k 个示例。

    target_sample:
        当前需要标注的样本，dict 包含 'input' 字段（及可选的 'id' 字段）。
    pool:
        候选示例列表，每个元素为 dict，包含 'input'、'output'（及可选的 'id'）字段。
    k:
        返回最相似示例的数量，默认为 5。

    Returns:
        按相似度从高到低排列的最多 k 个示例列表（已排除 target_sample 自身及无输出的样本）。
    """
    if not pool or k == 0:
        return []
